...
class UserServiceTest {
    ...
    void allUsersShouldIgnoreBigPhoto(@Mock UserRepository userRepository) {
        ...
        mainTestUser.setPhoto(bigPhotoData.getBytes(StandardCharsets.UTF_8));
        mainTestUser.setPhotoSmall(smallPhotoData.getBytes(StandardCharsets.UTF_8));
        ...
        when(userRepository.findByUsernameNot(eq(mainTestUserName)))
                .thenReturn(List.of(
                        new UserWithStatus(
                                ...
                                mainTestUser.getCurrency(),
                                mainTestUser.getFullname(),
                                mainTestUser.getPhotoSmall(),
        ...
        assertEquals(smallPhotoData, userJsonBulks.getFirst().photoSmall());
    }
    ...
    void getFriendsList(@Mock UserRepository userRepository) {
        ...
        when(userRepository.findFriends(eq(mainTestUser)))
        ...
        final UserJsonBulk invitation = friends.stream()
                .filter(u -> u.friendshipStatus() == INVITE_RECEIVED)
                .findFirst()
                .orElseThrow(() -> new AssertionError("Friend with state INVITE_SENT not found"));

        final UserJsonBulk friend = friends.stream()
                .filter(u -> u.friendshipStatus() == FRIEND)
                .findFirst()
    ...
    void getFriendsShouldReturnFriendsWithSpecifiedSearchQuery(@Mock UserRepository userRepository) {
        ...
        when(userRepository.findFriends(eq(mainTestUser), eq(secondTestUserName)))
                .thenReturn(Collections.singletonList(getMockUsersMappingFromDb().getFirst()));
    ...
    void getFriendsShouldReturnEmptyListWithUnknownSearchQuery(@Mock UserRepository userRepository) {
        ...
        when(userRepository.findFriends(eq(mainTestUser), eq("unknown")))
    ...
    @Test
    void getCurrentUserShouldReturnUserDetailsWhenUserExistsInDb(@Mock UserRepository userRepository) {
        //Assert
        when(userRepository.findByUsername(eq(mainTestUserName)))
        ...
        assertEquals(mainTestUserUuid, currentUser.id());
        assertEquals(mainTestUser.getFullname(), currentUser.fullname());
    ...
    void shouldCreateFriendshipRequest(@Mock UserRepository userRepository) {
        ...
        //Act
        UserJson userJson = userService.createFriendshipRequest(mainTestUserName, secondTestUserName);
    ...
    @Test
    void shouldNotCreateFriendshipRequestToSelf(@Mock UserRepository userRepository) {
        //Arrange
        userService = new UserService(userRepository);

        //Act
        SameUsernameException sameUsernameException = assertThrows(SameUsernameException.class,
                () -> userService.createFriendshipRequest(mainTestUserName, mainTestUserName)
        );

        //Assert
        assertEquals(sameUsernameException.getMessage(), "Can`t create friendship request for self user");
    }

    @Test
    void shouldAcceptFriendshipRequest(@Mock UserRepository repository) {
        //Arrange
        FriendshipEntity friendshipEntity = new FriendshipEntity();
        friendshipEntity.setRequester(secondTestUser);
        friendshipEntity.setStatus(FriendshipStatus.PENDING);
        mainTestUser.setFriendshipAddressees(Collections.singletonList(friendshipEntity));

        when(repository.findByUsername(eq(mainTestUserName)))
                .thenReturn(Optional.ofNullable(mainTestUser));
        when(repository.findByUsername(eq(secondTestUserName)))
                .thenReturn(Optional.ofNullable(secondTestUser));

        userService = new UserService(repository);

        //Act
        UserJson userJson = userService.acceptFriendshipRequest(mainTestUserName, secondTestUserName);
    ...
    @Test
    void shouldNotAcceptFriendshipRequestWithoutHavingInvitation(@Mock UserRepository repository) {
        //Arrange
        FriendshipEntity friendshipEntity = new FriendshipEntity();
        friendshipEntity.setRequester(secondTestUser);
        friendshipEntity.setStatus(FriendshipStatus.PENDING);
        mainTestUser.setFriendshipAddressees(List.of());
    ...
    @Test
    void shouldNotAcceptFriendshipRequestToSelf(@Mock UserRepository userRepository) {
        //Arrange
        userService = new UserService(userRepository);
        ...
        //Assert
        assertEquals(sameUsernameException.getMessage(), "Can`t accept friendship request for self user");
    }

    @Test
    void shouldDeclineFriendshipRequest(@Mock UserRepository repository) {
        //Arrange
        FriendshipEntity friendshipEntity = new FriendshipEntity();
        friendshipEntity.setRequester(secondTestUser);
        friendshipEntity.setStatus(FriendshipStatus.PENDING);
        mainTestUser.getFriendshipRequests().add(friendshipEntity);
        ...
        //Act
        UserJson userJson = userService.declineFriendshipRequest(mainTestUserName, secondTestUserName);
    ...
    @Test
    void shouldNotDeclineFriendshipRequestToSelf(@Mock UserRepository userRepository) {
        //Arrange
        userService = new UserService(userRepository);
        ...
        //Assert
        assertEquals(sameUsernameException.getMessage(), "Can`t decline friendship request for self user");
    }
